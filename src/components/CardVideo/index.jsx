...
export default function CardVideo() {
  ...
  const getVideos = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:3000/api/videos", `Bearer ${token}`);
  ...
  function dataAtualFormatada(createdAt){
        ...
        videosHome.map((video) => (
          <ContainerCard key={video._id}>
            ...
            <ContainerInfo>
              ...
              <DatePost>Postado em {dataAtualFormatada(video.createdAt)}</DatePost>
            </ContainerInfo>
          </ContainerCard>
        ))
      )}
